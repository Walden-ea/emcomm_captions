...
#         # self.lin2 = nn.Linear(128, hidden_size, bias=False)

#         # self.lin3 = nn.Linear(512, 256, bias=False)
#         # self.lin4 = nn.Linear(256, 128, bias=False)
...
class InformedSender(nn.Module):
    ...
    def forward(self, x, _aux_input=None):
        ...
        # return logits
        # print(f"Sender output h shape: {h.shape}")
        return h

    def return_embeddings(self, x):
        # embed each image (left or right)
        # print(f"x initial shape in sender embeddings: {x.shape}")
        x = x.permute(1, 0, 2)
        embs = []
        for i in range(self.game_size):
            h = x[i]
            if len(h.size()) == 3:
                h = h.squeeze(dim=-1)
            # print(f"h shape in sender embeddings loop: {h.shape}")
            # print(f"lin1 in sender embeddings: {self.lin1}")
            h_i = self.lin1(h)
            # h_i are batch_size x embedding_size
            h_i = h_i.unsqueeze(dim=1)
            h_i = h_i.unsqueeze(dim=1)
        ...
        # concatenate the embeddings
        h = torch.cat(embs, dim=2)
        # print(f"h shape in sender embeddings: {h.shape}")
        return h
...
class Receiver(nn.Module):
    def __init__(self, game_size, feat_size, embedding_size, vocab_size, reinforce):
        ...
        if reinforce:
            # self.lin2 = nn.Embedding(vocab_size, embedding_size)
            self.lin2 = nn.Linear(20, embedding_size, bias=False)
        else:
            self.lin2 = nn.Linear(vocab_size, embedding_size, bias=False)
